...
func TestNewSemver(t *testing.T) {
	s, err := NewSemVer(BASE_VERSION)
	if err != nil {
		t.Fatalf("Unable to allocate new SemVer object: %s", err)
	}
	...
	_, err2 := NewSemVer(INVALID_VERSION)
	if err2 == nil {
		t.Fatalf("Should throw error on invalid version.")
	}

	_, err3 := NewSemVer(EMPTY_VERSION)
	if err3 == nil {
...
func TestSemverString(t *testing.T) {
	...
	if s.String() != BASE_VERSION {
		t.Fatalf("SemVer doesn't match.  Got %s, looking for %s.", s.String(), BASE_VERSION)
	}

	s.Reset()
	s.ParseString(BUMP_MAJOR_VERSION)
	if s.String() != BUMP_MAJOR_VERSION {
		t.Fatalf("SemVer doesn't match.  Got %s, looking for %s.", s.String(), BUMP_MAJOR_VERSION)
	}

	s.Reset()
	s.ParseString(BUMP_MINOR_VERSION)
	if s.String() != BUMP_MINOR_VERSION {
		t.Fatalf("SemVer doesn't match.  Got %s, looking for %s.", s.String(), BUMP_MINOR_VERSION)
	}

	s.Reset()
	s.ParseString(BUMP_PATCH_VERSION)
	if s.String() != BUMP_PATCH_VERSION {
		t.Fatalf("SemVer doesn't match.  Got %s, looking for %s.", s.String(), BUMP_PATCH_VERSION)
	}

	s.Reset()
	s.ParseString(BUMP_PRERELEASE_VERSION)
	if s.String() != BUMP_PRERELEASE_VERSION {
		t.Fatalf("SemVer doesn't match.  Got %s, looking for %s.", s.String(), BUMP_PRERELEASE_VERSION)
	}

	s.Reset()
	s.ParseString(BUMP_BUILD_VERSION)
	if s.String() != BUMP_BUILD_VERSION {
		t.Fatalf("SemVer doesn't match.  Got %s, looking for %s.", s.String(), BUMP_BUILD_VERSION)
	}
}
...
func TestMinorSemVer(t *testing.T) {
	s, err := NewSemVer(BASE_VERSION)
	if err != nil {
		t.Fatalf("Unable to allocate new SemVer object: %s.", err)
	}
...
func TestPatchSemVer(t *testing.T) {
	s, err := NewSemVer(BASE_VERSION)
	if err != nil {
		t.Fatalf("Unable to allocate new SemVer object: %s.", err)
	}
...
func TestNonExplicitPrereleaseSemVer(t *testing.T) {
	s, err := NewSemVer(BASE_VERSION)
	if err != nil {
		t.Fatalf("Unable to allocate new SemVer object: %s.", err)
	}
...
func TestBuildSemVer(t *testing.T) {
	s, err := NewSemVer(BASE_VERSION)
	if err != nil {
		t.Fatalf("Unable to allocate new SemVer object: %s.", err)
	}
...
func TestAllFields(t *testing.T) {
	s, err := NewSemVer(ALL_FIELDS_VERSION)
	if err != nil {
		t.Fatalf("Unable to allocate new SemVer object: %s.", err)
	}
...
func TestEquals(t *testing.T) {
	...
	for i := range versions {
		v1, _ := NewSemVer(versions[i])
		v2, _ := NewSemVer(versions[i])
